...
export default function QuotationsPage() {
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const quotationsData = response?.data?.items || response?.data || response || [];
      setQuotations(quotationsData);
      
      // Set first quotation as selected if available
      if (quotationsData.length > 0 && !selectedQuote) {
        setSelectedQuote(quotationsData[0]);
      }
    } catch (err) {
  ...
  const filteredQuotations = quotations.filter(quote =>
    (quote.project_name || quote.projectName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    (quote.client_name || quote.client || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    (quote.quote_id || quote.quoteId || '').toLowerCase().includes(searchTerm.toLowerCase())
  );

  const calculateQuoteSummary = (quote) => {
    // If quote has sections (old structure), calculate from sections
    ...
    // If quote has API structure, use those values
    const baseAmount = quote.base_amount || quote.baseAmount || 0;
    const discountPercent = quote.discount_percent || quote.discountPercent || 0;
    const finalAmount = quote.final_amount || quote.finalAmount || 0;
    
    const discountAmount = (baseAmount * discountPercent) / 100;
    const subtotal = baseAmount - discountAmount;
    const gst = subtotal * 0.18;
    const total = finalAmount || (subtotal + gst);
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        {isLoading && (
          <div className="text-center py-10">
            <div className="text-gray-text">Loading quotations...</div>
          </div>
        )}
        ...
        {!isLoading && !error && quotations.length === 0 && (
          <div className="text-center py-10">
            <div className="text-gray-text">No quotations found. Create your first quotation!</div>
          </div>
        )}
        ...
        {!isLoading && filteredQuotations.length > 0 && (
          ...
          {filteredQuotations.map((quote) => (
            ...
            >
              ...
              <div className="p-5 border-b border-gray-border">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-white mb-1">
                      Project ID: {quote.project_id || quote.projectId || 'N/A'}
                    </h3>
                    <p className="text-sm text-gray-text mb-2">
                      <span className="font-medium">Client ID:</span> {quote.client_id || quote.clientId || 'N/A'}
                    </p>
                    <p className="text-sm text-gray-text">
                      <span className="font-medium">Base Amount:</span> ₹{(quote.base_amount || quote.baseAmount || 0).toLocaleString('en-IN')}
                    </p>
                    {quote.discount_percent > 0 && (
                      <p className="text-sm text-gray-text">
                        <span className="font-medium">Discount:</span> {quote.discount_percent}%
                      </p>
                    )}
                  </div>
                  <div className="text-right">
                    <div className="text-sm text-gray-text mb-2">
                      <div className="font-medium">Quote ID: {quote.id}</div>
                      <div>
                ...
                <div className="mt-3 pt-3 border-t border-gray-border">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-text text-sm">Final Amount:</span>
                    <span className="text-accent text-lg font-bold">
                      ₹{(quote.final_amount || quote.finalAmount || 0).toLocaleString('en-IN')}
                    </span>
                  </div>
                  {quote.notes && (
              ...
              {/* Quote Sections - Only show if available */}
          ...
          <>
            ...
            <div className="space-y-3 mb-6">
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-text">Base Amount</span>
                <span className="text-white font-medium">₹{summary.subtotal.toLocaleString('en-IN')}</span>
              </div>
              {summary.discount > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-text">Discount ({summary.discountPercent}%)</span>
                  <span className="text-white font-medium">-₹{summary.discount.toLocaleString('en-IN')}</span>
                </div>
              )}
              {summary.designFee && (
                <div className="flex items-center justify-between text-sm">
